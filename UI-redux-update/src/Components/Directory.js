...
import {getCurrentDir, getRootDirPath, setCurrentDirectory} from '../redux/directoriesSlice';
...
const Directory = (props) => {
	...
	const rootDir = useSelector(getRootDirPath);
	...
	const handleClickDir = dirName => {
		dispatch(setCurrentDirectory(currentDir.path.concat('/', dirName)));
	};
		...
		return (
			<Box key={0} my={1}>
				<FileLink name={'../'} handleClick={event => {
					...
					dispatch(setCurrentDirectory(destinationPath.join('/')));
				}} />
			</Box>
		);
	};
